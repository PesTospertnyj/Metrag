...
class CompareController extends Controller {
	...
	public function actionCompareitems($start = 0) {
		...
		try {
			$items = Parser::find()->orderBy('id DESC')->offset($start)->limit($comparingItems)->all();
			foreach ($items as $item) {
				...
				self::logDB("SELECT COUNT(*) FROM `apartment` WHERE ({$sql}):" . $similarPhonesCount);
