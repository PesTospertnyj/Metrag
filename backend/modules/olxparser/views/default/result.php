                ...
                'buttons' =>
                    [
                        'update' => function ($url, $model, $key) {
                           return '<a href="'.Url::base(true).'/apartment/add-from-parser?id='.$key.'" title="Добавить" class="btn btn-success" style="color: #000" aria-label="Добавить" data-pjax="0" target="_blank"><span class="glyphicon glyphicon-pencil"></span></a>';
                    },
                    /*    'delete' => function ($url, $model, $key) {
                        ...
                        'delete' => function ($url, $model, $key) {
                            ...
                            $pattern_sort = '/sort=(-?\w*)/';
                            $url = Url::current();
                            if(preg_match($pattern_page, $url, $matches))
                            {
                                $page = $matches[1];
                            }
                            if(preg_match($pattern_sort, $url, $matches))
                            {
                                $sort = $matches[1];
                            }
                            return '<a href="'.Url::base(true).'/olxparser/parser/delete?id='.$key.'&page='.$page.'&sort='.$sort.'" title="Удалить" aria-label="Удалить" data-pjax="0" data-confirm="Вы уверены, что хотите удалить этот элемент?" data-method="post" class="btn btn-danger" style="color: #000" aria-label="Добавить" data-pjax="0" ><span class="glyphicon glyphicon-trash"></span></a>';
